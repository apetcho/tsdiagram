...
function tsdiagram(salt,temp, p, addscatter)
%% generating background density contours
if nargin < 3
    error('Insufficient number of inputs. At least 3 inputs are needed')
elseif nargin == 3
    addscatter = 0;
end
...
[mt, nt] = size(temp);
[mp, np] = size(p);
fprintf('size(salt) = [%2d, %2d]', ms, ns);
fprintf('size(temp) = [%2d, %2d]', mt, nt);
fprintf('size(p) = [%2d, %2d]', mp, np);
% check the dimensions of salt and temp
if (ms ~= mt) || ( ns ~= nt)
   error('The first two input must have the dimensions') 
end
temp = temp(:);
salt = salt(:);
smin = nanmin(salt)-0.01.*nanmin(salt);
smax = nanmax(salt)+0.01.*nanmax(salt);
tmin = nanmin(temp)-0.1*nanmax(temp);
tmax = nanmax(temp)+0.1*nanmax(temp);
xdim = round((smax-smin)./0.1+1);
ydim = round((tmax-tmin)+1);
% rho = zeros(ydim,xdim);
tempi=((1:ydim)-1)*1+tmin;
salti=((1:xdim)-1)*0.1+smin;
disp(xdim);
disp(ydim);
[X, Y] = meshgrid(salti, tempi);

% Broadcast the value of p if necessary or reject it.
if (mp == 1) && (np == 1)
    p = p * ones(size(X));
elseif (mp == ms) && (np == 1)
    p = repmat(p, [1, ns]);
...
rho = gsw_rho(X, Y, nanmean(nanmean(0*p))) - 1000;

[c,h]=contour(X,Y, rho,'k--');
% clabel(c,h,'LabelSpacing',1000);
clabel(c, 'FontSize', 20)
set(h, 'LineWidth',3,'LabelSpacing',1000)
